...
{
    uint16_t payload_sz;
    if ((hdr[MCTT_GID_INDEX] == MCTT_SE_GID_HEARDER)) {
        payload_sz = (uint16_t)(MCTT_UCI_HEADAER + hdr[MCTT_DATA_PAYLOD_LENGTH_INDEX]);
    }
    else if ((hdr[MCTT_GID_INDEX] == MCTT_DATA_SEND) ||
        (hdr[MCTT_GID_INDEX] == MCTT_LL_DATA_SEND)){
        // For data packets, payload length is always encoded in byte 3 and4 (index 2 and 3), the UCI payload extension.
        payload_sz = (uint16_t)(MCTT_UCI_HEADAER + (hdr[MCTT_DATA_PAYLOD_LENGTH_INDEX] +
                                                       (hdr[MCTT_DATA_PAYLOD_LENGTH_INDEX + 1] << 8)));
    }
    else {
        payload_sz = (uint16_t)(MCTT_UCI_HEADAER + hdr[MCTT_UCI_PAYLOD_LENGTH_INDEX]);
    }
    if (hdr[1] & 0x80)
...
{
    uint8_t headerSize = HEADER_SIZE_CDC;
