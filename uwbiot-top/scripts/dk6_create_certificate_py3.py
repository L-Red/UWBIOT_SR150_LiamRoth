...
def write_cert_c_file(
    c_file_name, customer_id, device_min, device_max, exported_key, sign_key
):
    ...
    with open(c_file_name, 'w') as source:
        ...
        for i, x in enumerate(
            struct.unpack('<64L',
                          number.long_to_bytes(public_key.n)[::-1])
        ):
            if i % 4 == 0:
                source.write('\n        ')
...
def write_cert_bin_file(
    bin_file_name, customer_id, device_min, device_max, exported_key, sign_key
):
    ...
    certificate = certificate_struct.pack(
        CERTIFICATE_MARKER, 0, 0, 0, customer_id, device_min, device_max
    )
...
parser = argparse.ArgumentParser(
    description=
    'DK6 Certificate Generator: for JN5189, QN9090, K32W061 processors'
)
...
if args.key is None:
    print(
        "Key File name must be provided to sign the certificate that is to be generated"
    )
else:
    try:
        ...
        if os.path.isdir(args.cert_path) == False:
            try:
                ...
                write_cert_bin_file(
                    public_cert_bin_file, args.customer, args.device_min,
                    args.device_max, exported_key, key
                )
        else:
            print(
                args.cert_path +
                " already exists! Provide another path or destroy it first"
            )
