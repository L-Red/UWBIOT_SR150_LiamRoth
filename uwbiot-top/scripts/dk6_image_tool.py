...
def parse_sections(file):
    ...
    Section = namedtuple(
        'Section',
        ['idx', 'name', 'size', 'vma', 'lma', 'offset', 'align', 'flags']
    )
...
def print_sota_img_directory_cmd(blobNumber):
    ...
    for i in range(1, 9):
        ...
        if i == 1:
            bootable = "01"
        position_in_flash = get_symbol_value(
            elf_file_name, "m_blob_position" + str(i)
        )
        position_in_flash = '%0*X' % (2, position_in_flash)
        blobTargetAddr = get_symbol_value(
            elf_file_name, "m_blob" + str(i) + "_start"
        )
        blobTargetAddr = '%0*X' % (8, blobTargetAddr)
        blobStatedSize = get_symbol_value(
            elf_file_name, "m_blob" + str(i) + "_size"
        )
...
def BuildImageElfDeprecated(args, elf_file_name, bin_file_name, verbose):
    ...
    dump_section = subprocess.check_output(
        [
            'arm-none-eabi-objcopy', '--dump-section',
            '%s=data.bin' % last_section.name, args.in_file
        ]
    )

    if args.appcore_image is True:
        boot_block = boot_block_struct.pack(
            BOOT_BLOCK_MARKER, 1, args.target_appcore_addr,
            image_size + boot_block_struct.size, 0, 0, 0
        )
    else:
        boot_block = boot_block_struct.pack(
            BOOT_BLOCK_MARKER, 0, 0, image_size + boot_block_struct.size, 0, 0,
            0
        )
    ...
    update_section = subprocess.check_output(
        [
            ...
            '%s=data.bin' % last_section.name, args.in_file, args.out_file
        ]
    )
    ...
    with open(args.out_file, 'r+b') as elf_file:
        ...
        print(
            "Writing checksum {:08x} to file {:s}".format(
                vectsum, args.out_file
            )
        )
...
######################
#
# 2021.03.18 : <Param> Changed elf_bin_file to elf_file_name
#            :
#            : <Inline> Create _<bin_file_name>_data.bin instead of data.bin
#
def BuildImageElf(args, elf_file_name, bin_file_name, verbose):  # +2021.03.18
    is_signature = False
    error = 0
    if args.signature_path is not None:
        ...
        if os.path.isdir(sign_dir_path) == False:
            sign_dir_path = os.path.join(
                os.path.dirname(__file__), args.signature_path
            )
            if os.path.isdir(sign_dir_path) == False:
                error = 1
            else:
    ...
    if args.compatibility_list is not None:
        ...
        compatibility_list = [
            list(map(auto_int, compatibility_item.split(",")))
        ...
        for i in range(len(compatibility_list)):
            ...
            for j in range(len(item)):
                if j == 1:
                    blobIdCompatibilityList = "     Blob ID 0x=" + '%0*X' % (
                        8, item[j - 1]
                    )
                    print(
                        "Blob ID =0x" + '%0*X' % (4, item[j - 1]) +
                        " - version =0x" + '%0*X' % (8, item[j])
                    )
        compatibility_len = len(
            compatibility_list
        ) * compatibility_struct.size + compatibility_len_struct.size
        if verbose:
    ...
    if args.sota_number_of_blob is not None and (
        compatibility_len // compatibility_struct.size
    ) != args.sota_number_of_blob - 1:
        print(
            "!!! Error the compatibility list length must be = to the number of blobs -1 : "
            + str(compatibility_len / compatibility_struct.size) + "(len) != " +
            str(args.sota_number_of_blob - 1)
        )
        error = 1
    ...
    if verbose:
        print(
            "Last Section LMA={:08x} Size={:08x}".format(
                last_section.lma, last_section.size
            )
        )
    ...
    with open(args.out_file, 'r+b') as elf_file:
        ...
        print(
            "Writing CRC32 of header {:08x} to file {:s}".format(
                fields[10], args.out_file
            )
        )
    ...
    dump_section = subprocess.check_output(
        [
            ...
            '--dump-section',
            '%s=_%s_data.bin' %
            (last_section_name, bin_file_name),  # +2021.03.18
            args.out_file
        ]
    )
    ...
    if args.compatibility_list is not None:
        ...
        compatibility = compatibility_len_struct.pack(
            len(compatibility_list)
        ) + b''.join(
            [
                compatibility_struct.pack(*compatibility_item)
                for compatibility_item in compatibility_list
            ]
        )
    ...
    if (args.certificate is True):
        ...
        if (os.path.isfile(certif_file_path)):
            ...
            if actual_len != expected_cert_len:
                print(
                    "Certificate error expected expected %d got %d" %
                    (expected_cert_len, actual_len)
                )
                error = 1
    ...
    img_total_len = boot_block_offset + boot_block_struct.size + len(
        certificate
    )
    ...
    if (is_signature == True):
        ...
        message = header + input_file[header_struct.
                                      size:] + boot_block + certificate
        hash = SHA256.new(message)

        out_file_path = os.path.join(
            os.path.dirname(__file__), 'dump_python.bin'
        )
    ...
    with open('_%s_data.bin' % bin_file_name, 'ab') as out_file:  # +2021.03.18
        ...
        out_file.write(
            compatibility + padding_bytes + boot_block + certificate + signature
        )
    if verbose:
    ...
    if out_file_sz > stated_size:
        print(
            "Error: Binary file size ({:08x}) must be less or equal to stated size {:08x}"
            .format(os.stat(bin_file_name).st_size, stated_size)
        )
        error = 1
    if out_file_sz != img_total_len:
        print(
            "File size %d different from expected %d" %
            (out_file_sz, img_total_len)
        )
        error = 1
    return error
...
parser.add_argument(
    'in_file',
    help=
    "Binary to be post-processed: generating header and optionally appending certificate and/or signature."
)
...
parser.add_argument(
    ...
    '--signature_path',
    help=
    "Sets directory from which certificate and private key are to be retrieved"
)
parser.add_argument(
    '-k',
    '--key',
    action='store_true',
    help=
    "2048 bits RSA private key in PEM format used to sign the full image. If -c option is used the full image includes the certificate + the signature of the certificate. The key shall be located in the same directory as the image_tool script. See priv_key.pem example."
)
parser.add_argument(
    '-p',
    '--password',
    help=
    "This is the pass phrase from which the encryption key is derived. This parameter is only required if key provided through the -k option is a PEM encrypted key."
)
parser.add_argument(
    '-c',
    '--certificate',
    action='store_true',
    help=
    "When option is selected, the certificate cert.bin is appended to the image."
)
parser.add_argument(
    '-i',
    '--image_identifier',
    type=int,
    help=
    "This parameter is to set the archive identifier. 0: SSBL or legacy JN518x/QN9090 applications, loaded at 0x00000000. 1: application (ZB) loaded at address 0x00004000 by default. 2: application (BLE) image loaded at address 0x00053000 by default"
)
parser.add_argument(
    '-a',
    '--appcore_image',
    action='store_true',
    help=
    "This parameter is only relevant if dual application (app1 image) shall reside in flash. Do not use in conjunction with -i option."
)
parser.add_argument(
    '-t',
    '--target_addr',
    type=int,
    help=
    "Target address of image. Used in conjunction with -i option to override the default set by image identifier, or with -a option to specify address of the appcore image (app1 image)."
)
parser.add_argument(
    '-s',
    '--stated_size',
...
parser.add_argument(
    ...
    type=auto_int,
    default=0,
    help="Image version. Default is 0."
)
parser.add_argument(
    '-b',
    '--verbose',
...
parser.add_argument(
    ...
    type=int,
    help=
    "This parameter is used to generate the image directory command to be provisioned"
)
parser.add_argument(
    '-bid',
    '--blob_id',
    type=auto_int,
    help=
    "This parameter is to add a blob id. Can be used only if the sota arg is given"
)
...
try:
    ...
    print("Warning! Build failed. Can not build '%s'" % bin_file_name)
    print("Warning! Error: %s" % ex)
    error = 1

if error != 0:
    import time
    time.sleep(1)  # +2021.03.18
    # Sleep for few seconds, else ``make -j`` fails
    if os.path.exists(elf_file_name):
        os.remove(elf_file_name)
    if os.path.exists(out_file_path):
        os.remove(out_file_path)
if os.path.exists("_" + bin_file_name + "_data.bin"):
    os.remove("_" + bin_file_name + "_data.bin")  # +2021.03.18
