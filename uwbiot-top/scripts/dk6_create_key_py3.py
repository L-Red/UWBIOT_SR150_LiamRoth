...
def write_psect_img_pk_write_cmd(cmd):
    with open(cmd, 'w') as psectcmd:
        ...
        psectcmd.write(
            'echo "Only use this command to set or replace the public key when :";'
        )
        psectcmd.write(
            'echo "    1) a direct authentication method is used or";'
        )
        psectcmd.write(
            'echo "    2) when indirect authentication method is used it will set/replace the key that is used to authenticate the certificate containing the key.";'
        )
...
if os.path.isdir(args.key_path) == False:
    try:
        ...
        with open(private_key_file, 'w') as key_file:
            key_file.write(
                (key.exportKey('PEM', args.password)).decode('utf-8')
            )
