...
{
    uint8_t *pmsgdata;
    pmsgdata = (uint8_t *)(uwb_cb.pLast_cmd_buf + 1) + uwb_cb.pLast_cmd_buf->offset;
    ...
    if ((uwb_cb.IsConformaceTestEnabled == TRUE) && (*pmsgdata == UCI_DPF_SND)) {
        ...
        * when we send the message data it expects the Ntf which is handled by async for mctt/pctt .
        */
...
// Chaining is enabled
...
#endif // UWBFTR_ChainedUCI
            {
                 ...
                 */
                if ((ps[0] == UCI_DPF_SND) && (uwb_cb.UwbOperatinMode == kOPERATION_MODE_mctt)) {
...
void chain_data_packet(UWB_HDR *p_msg)
{
    ...
    if (!uwb_cb.IsConformaceTestEnabled && pbf) { // No chaining required for conformance test mode
        if (!chained_packet.is_first_frgmnt_done) {
            ...
            if (payload_length <= (sizeof(chained_packet.buffer) - chained_packet.offset)) {
                phOsalUwb_MemCopy(&chained_packet.buffer[chained_packet.offset], pp, payload_length);
...
// Chaining is enabled
            ...
            *        ex:
            *           1: We will receive the notifications as chunks and pbf bit enabled(see UCI_MSG_PRS_PBF for more).
            *           2: Chunk the data till pbf bit disabled.
            *           3: PBF bit Enable is indication of continuous NTF/Rsp of same GID and OID.
            *           4: PBF is Disable is indication of either last NTF/Rsp of same GID and OID or Rsp/NTF is less than or equal to 0xFF.
            */
            if (!uwb_cb.IsConformaceTestEnabled && pbf) { // No chaining required for conformance test mode
                if (!chained_packet.is_first_frgmnt_done) {
       ...
       * command window */
        ...
        case UCI_MT_NTF:
            ...
            if ((gid == UCI_GID_PROPRIETARY) || (gid == UCI_GID_PROPRIETARY_SE) || (gid == UCI_GID_INTERNAL_GROUP) ||
                (gid == UCI_GID_VENDOR)) {
                ...
                if ((gid == UCI_GID_RANGE_MANAGE) && (oid == UCI_MSG_DATA_CREDIT_NTF)) {
                    if (pp[CREDIT_AVAILABILITY_INDICATOR_OFFSET]) {
                        /* In Case of Data transfer, Currently first a credit notification with status credit not available
                         * comes and then data transmit done notification, followed by a credit notification with credit
                         * available, update cmd window only if credit is available
                         */
                    ...
                    if ((oid == UCI_MSG_CORE_DEVICE_STATUS_NTF) &&
                        ...
                        /*After receving FC or FF retry will happen only once to get the DEVICE_STATUS
                        ...
                        */
                        ...
                        uwb_stop_quick_timer(); /*stop the pending timer */
                        UCI_TRACE_W(
                            "Retrying last sent command as FW asked for retry (CORE_GENERIC_ERROR_NTF -> "
                            "STATUS_MESSAGE_RETRY)");
                        uint8_t *pLast_buf;
                        pLast_buf = (uint8_t *)(uwb_cb.pLast_cmd_buf + 1) + uwb_cb.pLast_cmd_buf->offset;
                        if ((((*pLast_buf & UCI_MT_MASK) >> UCI_MT_SHIFT) == UCI_MT_DATA) && ((*pLast_buf & UCI_GID_MASK) == UCI_DPF_SND))
                        {
