...
UWBStatus_t uwb_fwdl_getFwImage(phUwbFWImageContext_t *pfwImageCtx)
{
    NXPLOG_UWB_FWDNLD_I("FWDL Directly from host");
    if (pfwImageCtx->fwImage == NULL || pfwImageCtx->fwImgSize <= 0) {
        return kUWBSTATUS_FAILED;
    }

    setFwImage(pfwImageCtx->fwImage, pfwImageCtx->fwImgSize);
    NXPLOG_UWB_FWDNLD_D("Firmware size: %d", pfwImageCtx->fwImgSize);
...
UWBStatus_t uwb_fwdl_downloadFw(uwb_fwdl_provider_t *pCtx)
{
    ...
    do {
        ...
        if (dataSz) {
            ...
            rcvLen    = PHHBCI_MAX_LEN_DATA_MISO;
            (void)phTmlUwb_hbci_transcive((uint8_t *)&pCtx->uwb_fwdl_MosiApdu.payload[PHHBCI_LEN_HDR],
                (uint16_t)payloadSz,
